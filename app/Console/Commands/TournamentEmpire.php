...
class TournamentEmpire extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'tournament:empire';
     ...
     * @var string
     */
    protected $description = 'scrape Empire tournies';
        ...
        $html = HtmlDomParser::file_get_html( 'http://www.thepokerroom.co.uk/poker-room-2/tournaments/' );
            ...
            foreach ($row as $r) {
                ...
                if (strlen($date) > 3 && strlen($date) < 9 && $this->thisMonth($date)) {
                    ...
                    $check = Tournaments::where('casino','=','The Empire')
                                                ->where('event', '=', $event)
